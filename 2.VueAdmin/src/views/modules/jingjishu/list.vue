<template>
	<div class="main-content" :style='{"color":"#3B3E40","padding":"30px 30px 30px","fontSize":"14px"}'>
			...
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"0 10px","margin":"0 0 20px","overflow":"hidden","borderRadius":"0px","flexWrap":"wrap","background":"none","display":"flex","fontSize":"inherit","justifyContent":"space-between"}' :inline="true" :model="searchForm">
				<el-row :style='{"border":"0px solid #e3e3e3","padding":"10px 0","borderRadius":"5px","textAlign":"center","flexWrap":"wrap","background":"#fff","flexDirection":"row","display":"flex","width":"auto","fontSize":"inherit","justifyContent":"flex-start","order":"2"}' >
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">书名</label>
						<el-input v-model="searchForm.bookname" placeholder="书名" @keydown.enter.native="search()" clearable></el-input>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}' class="select" label="书籍类型" prop="shujileixing">
						...
						<el-select clearable v-model="searchForm.shujileixing" placeholder="请选择书籍类型" >
							<el-option v-for="(item,index) in shujileixingOptions" v-bind:key="index" :label="item" :value="item"></el-option>
						</el-select>
					</div>
					<div :style='{"margin":"0 10px 0 0","fontSize":"inherit","flexWrap":"wrap","display":"flex"}'>
						<label :style='{"margin":"0 10px 0 0","color":"inherit","display":"inline-block","lineHeight":"40px","fontSize":"inherit","fontWeight":"500","height":"40px"}' class="item-label">出版社</label>
				...
				<el-row class="actions" :style='{"border":"0px solid #e3e3e3","padding":"10px 0","margin":"0","color":"#fff","textAlign":"left","flexDirection":"row","display":"flex","justifyContent":"flex-start","borderRadius":"5px","flexWrap":"wrap","background":"#fff","width":"auto","fontSize":"inherit"}'>
					<el-button class="add" v-if="isAuth('jingjishu','新增')" type="success" @click="addOrUpdateHandler()">
						<span class="icon iconfont icon-tianjia13" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						添加
					</el-button>
					<el-button class="del" v-if="isAuth('jingjishu','删除')" :disabled="dataListSelections.length?false:true" type="danger" @click="deleteHandler()">
					...
					<el-button class="btn18" v-if="isAuth('jingjishu','爬取数据')" type="success" @click="spider()">
						<span class="icon iconfont icon-shuju17" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						爬取数据
					</el-button>


					<el-button class="btn18" v-if="isAuth('jingjishu','书籍类型')" type="success" @click="chartDialog1()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						书籍类型
					</el-button>
					<el-button class="btn18" v-if="isAuth('jingjishu','出版社图书')" type="success" @click="chartDialog2()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						出版社图书
					</el-button>
					<el-button class="btn18" v-if="isAuth('jingjishu','图书量情况')" type="success" @click="chartDialog3()">
						<span class="icon iconfont icon-a-fenxiang2" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block"}'></span>
						图书量情况
					</el-button>
				</el-row>
			...
			<div :style='{"border":"0px solid #e9e9e9","width":"100%","padding":"0px","margin":"10px 0 0","borderRadius":"5px","background":"rgba(255,255,255,1)"}'>
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column :resizable='true' :sortable='true' prop="cover" width="200" label="封面">
						<template slot-scope="scope">
							<div v-if="scope.row.cover">
								<img v-if="scope.row.cover.substring(0,4)=='http'" :src="scope.row.cover.split(',')[0]" width="100" height="100">
								<img v-else :src="$base.url+scope.row.cover.split(',')[0]" width="100" height="100">
							</div>
							<div v-else>无图片</div>
						</template>
					</el-table-column>
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							<el-button class="view" v-if=" isAuth('jingjishu','查看')" type="success" @click="addOrUpdateHandler(scope.row.id,'info')">
							...
							<el-button class="btn8" v-if="isAuth('jingjishu','图书借阅')" @click="tushujieyueCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
							...
							<el-button class="btn8" v-if="isAuth('jingjishu','书籍入库')" @click="shujirukuCrossAddOrUpdateHandler(scope.row,'cross','','','')" type="success">
								<span class="icon iconfont icon-xihuan" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block","height":"auto"}'></span>
								书籍入库
							</el-button>
							<el-button class="edit" v-if=" isAuth('jingjishu','修改') " type="success" @click="addOrUpdateHandler(scope.row.id)">
							...
							<el-button class="view" v-if="isAuth('jingjishu','查看评论')" type="success" @click="disscussListHandler(scope.row.id)">
								<span class="icon iconfont icon-chakan14" :style='{"margin":"0 2px","fontSize":"inherit","color":"inherit","display":"inline-block","height":"auto"}'></span>
								查看评论
							</el-button>
							...
							<el-button class="del" v-if="isAuth('jingjishu','删除') " type="primary" @click="deleteHandler(scope.row.id )">
		...
		<shujiruku-cross-add-or-update v-if="shujirukuCrossAddOrUpdateFlag" :parent="this" ref="shujirukuCrossaddOrUpdate"></shujiruku-cross-add-or-update>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shujileixingChart1" style="width:100%;height:600px;"></div>
		...
		<el-dialog
		  ...
		  width="800">
			<div id="chubansheChart2" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="shuliangChart3" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import shujirukuCrossAddOrUpdate from "../shujiruku/add-or-update";
	export default {
		data() {
			return {
				shujileixingOptions: [],
				...
				shujirukuCrossAddOrUpdateFlag: false,
		...
		methods: {
			...
			tushujieyueCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				...
				this.$storage.set('crossTable','jingjishu');
			...
			shujirukuCrossAddOrUpdateHandler(row,type,crossOptAudit,statusColumnName,tips,statusColumnValue){
				this.showFlag = false;
				this.addOrUpdateFlag = false;
				this.shujirukuCrossAddOrUpdateFlag = true;
				this.$storage.set('crossObj',row);
				this.$storage.set('crossTable','jingjishu');
				this.$storage.set('statusColumnName',statusColumnName);
				this.$storage.set('statusColumnValue',statusColumnValue);
				...
				if(statusColumnName!=''&&!statusColumnName.startsWith("[")) {
					...
					for (var o in obj){
						if(o==statusColumnName && obj[o]==statusColumnValue){
							...
							this.showFlag = true;
							this.shujirukuCrossAddOrUpdateFlag = false;
							return;
						}
					}
				}
				this.$nextTick(() => {
					this.$refs.shujirukuCrossaddOrUpdate.init(row.id,type);
				});
			},
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var shujileixingChart1 = echarts.init(document.getElementById("shujileixingChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                shujileixingChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    shujileixingChart1.resize();
                };
            }
        });
      })
...
//统计接口
    chartDialog2() {
      ...
      this.$nextTick(()=>{
        ...
        var chubansheChart2 = echarts.init(document.getElementById("chubansheChart2"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                chubansheChart2.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    chubansheChart2.resize();
                };
            }
        });
      })
...
//统计接口
    ...
    getDataList() {
      ...
      let params = {
        ...
        order: 'desc',
           ...
           if(this.searchForm.bookname!='' && this.searchForm.bookname!=undefined){
	...
	this.$router.push({path:'/discussjingjishu',query:{refid:id}});
    },
    // 下载
    download(file){
	...
	jingjishustatusChange(e,row){
		if(row.status==0){
			row.passwordwrongnum = 0
		}
		this.$http({
			url:'jingjishu/update',
			method:'post',
			data:row
		}).then(res=>{
			if(row.status==1){
