...
<div>
    ...
    <el-form :inline="true" :model="formSearch" class="list-form-pv" :style='{"row-gap":"10px","padding":"24px 0","alignItems":"center","flexWrap":"wrap","display":"flex","width":"100%","height":"auto"}'>
      ...
      <el-form-item :style='{"margin":"0 10px"}'>
        ...
        <el-select v-model="formSearch.shujileixing" placeholder="请选择书籍类型" :clearable="true">
          <el-option v-for="(item, index) in shujileixingOptions" :key="index" :label="item" :value="item"></el-option>
        </el-select>
      </el-form-item>
      <el-form-item :style='{"margin":"0 10px"}'>
	  ...
	  <el-button v-if="btnAuth('jingjishu','新增')" :style='{"cursor":"pointer","border":"1px solid","padding":"0px 15px","margin":"0 10px 0 0","outline":"none","color":"#000","borderRadius":"4px","background":"none","width":"auto","fontSize":"14px","lineHeight":"42px","height":"42px"}' type="primary" @click="add('/index/jingjishuAdd')"><i v-if="true" :style='{"color":"#000","margin":"0 10px 0 0","fontSize":"14px"}' class="el-icon-circle-plus-outline"></i>添加</el-button>
    </el-form>
		  ...
		  <div v-for="(item, index) in dataList" :key="index" @click.stop="toDetail(item)" class="list-item animation-box">
		    ...
		    <div :style='{"padding":"0 10px","overflow":"hidden","flex":"1","display":"flex","height":"auto"}' class="item-info">
		      <div :style='{"width":"50%","padding":"0 10px 0 0","color":"#999","flex":"1"}'>
				...
				<div class="title">{{item.chubanyear}}</div>
				<div class="title">{{item.chubanshe}}</div>
		      ...
		      <div class="desc">{{item.laiyuan}}</div>
		    </div>
		  </div>
		</div>
	</div>
...
<script>
  export default {
    ...
    created() {
		...
		this.$http.get('option/shujileixing/shujileixing').then(res => {
			if (res.data.code == 0) {
    ...
    methods: {
      ...
      getFenlei() {
		...
		this.feileiColumn = 'shujileixing'
      },
      getList(page, fenlei, ref = '') {
        ...
        if (this.curFenlei != '全部') searchWhere.shujileixing = this.curFenlei;
		if (this.sortType) searchWhere.sort = this.sortType
		if (this.sortOrder) searchWhere.order = this.sortOrder
        this.$http.get(`jingjishu/${this.centerType?'page':'list'}`, {params: Object.assign(params, searchWhere)}).then(res => {
          if (res.data.code == 0) {
      ...
      toDetail(item) {
        ...
        this.$router.push({path: '/index/jingjishuDetail', query: params});
      },
